...
class Time_tracker:
    ...
    def set_task_index(self,task_index):
        ...
        if self.vis_time and self.qry_time and self.sub_time:
            write_time_to_file(self.current_task_index,self.vis_time,self.qry_time,self.sub_time)
        else:
    ...
    def get_time(self,which_time=str):
        # 计算时间
        self.this_time = self.end_time - self.start_time if self.end_time else time.time() - self.start_time
        ...
        if which_time == 'visit':
            ...
            self.qry_time = self.this_time + self.qry_time if self.qry_time else self.this_time
        elif which_time == 'submit':
            self.sub_time = self.this_time + self.sub_time if self.sub_time else self.this_time
